...
function getAllMenu(){
	return	array(
			...
			'order' => array('name' => '订单管理', 'icon'=>'fa-money', 'sub_menu' => array(
					array('name' => '实物订单列表', 'act'=>'index', 'control'=>'Order'),
          array('name' => '团购券订单列表', 'act'=>'supply_list', 'op'=>'Order'),
          array('name' => '抽奖订单列表','act'=> 'luck_draw_list','op'=>'Order'),
...
function getMenuArr(){
  ...
  $isSupplier = isSupplier($admin_info);
  $menuArr = include APP_PATH.'admin/conf/menu.php';
  if($isSupplier){
    $menuArr = array('shop'=>array('name'=>'商城','child'=>array(
        array('name' => '团购券','child' => array(
            array('name' => '供应商团购券', 'act'=>'index', 'op'=>'SupplierOrder'),
        )),
    )));
  };
	...
	if($act_list != 'all' && !empty($act_list)){
		...
		foreach($menuArr as $k=>$val){
            ...
            if(strpos($role_right,$son['op'].'Controller@'.$son['act']) !== 0){
...
function isSupplier($user){
  ...
  $role = M('admin_role')->where("role_id", $role_id)->find();

  return ($role['role_name'] == "供应商");

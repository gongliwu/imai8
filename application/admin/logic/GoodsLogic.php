...
class GoodsLogic extends Model
{
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        ...
        foreach ($spec_arr_sort as $key =>$val)
        {
         ...
         $keySpecGoodsPrice = M('SpecGoodsPrice')->where('goods_id = '.$goods_id)->getField('key,key_name,price,exchange_integral,store_count,bar_code,sku');//规格项
       ...
       foreach ($spec_arr2 as $k => $v) 
       {
            ...
            $keySpecGoodsPrice[$item_key][exchange_integral] ? false : $keySpecGoodsPrice[$item_key][exchange_integral] = 0; // 兑换积分默认为0
            ...
            $str .="<td><input name='item[$item_key][exchange_integral]' value='{$keySpecGoodsPrice[$item_key][exchange_integral]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";

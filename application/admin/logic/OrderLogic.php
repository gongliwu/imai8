...
class OrderLogic extends Model
{
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        $res = M('order')->alias('o')->where($condition)->limit("$start,$page_size")->order($order)->select();
    ...
    public function deliveryHandle($data){
        ...
        // 配送方式
        $shipping = M('shipping')->where('shipping_code', $data['shipping_code'])->find();
        if (empty($shipping)) {
            return False;
        }
        ...
        $updata['shipping_code'] = $shipping['shipping_code'];
        $updata['shipping_name'] = $shipping['shipping_name'];

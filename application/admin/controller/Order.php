...
class Order extends Base {
     ...
     * 团购券订单
     */
    public function ajax_supply_list(){
        $p = I('p/d',1);
        $orderLogic = new OrderLogic();
        $timegap = I('timegap');
        if($timegap){
        ...
        $count = M('order')->alias('o')->join('__ORDER_GOODS__ og','og.order_id = o.order_id')
        ...
        $show = $Page->show();
        //获取订单列表
        $orderList = M('order')->alias('o')->join('__ORDER_GOODS__ og','og.order_id = o.order_id')
                 ->field("o.*,og.goods_name,og.goods_num")
                 ->where($condition)->page($p,10)->order('o.order_id DESC')->select();
        $this->assign('orderList',$orderList);
        $this->assign('page',$show);// 赋值分页输出
        $this->assign('pager',$Page);
        return $this->fetch();
    ...
    public function luck_draw_list(){
      $begin = date('Y-m-d',strtotime("-1 year"));//30天前
    	$end = date('Y/m/d',strtotime('+1 days'));
    	$this->assign('timegap',$begin.'-'.$end);
      return $this->fetch();
     ...
     * 抽奖订单列表
     */
    public function ajax_luck_draw_list(){
        $orderLogic = new OrderLogic();
        $timegap = I('timegap');
        if($timegap){
          $gap = explode('-', $timegap);
    ...
    */
    ...
    public function detail($order_id){
        ...
        // 获取操作记录
        $action_log = M('order_action')->where(array('order_id'=>$order_id, 'action_user'=>array('>',0)))->order('log_time desc')->select();
        $userIds = array();
        //查找用户昵称
        ...
        $delivery_data = array();
        $delivery_doc = M('delivery_doc')->where(array('order_id'=>$order['order_id']))->order('id desc')->find();
        if ($delivery_doc) {
            $customer = '062D123C29131C852F7D4662466B586D';
            $key = 'qsQlBNXy4270';

            $url = 'http://poll.kuaidi100.com/poll/query.do';
            $param = '{"com":"' . $delivery_doc['shipping_code'] . '", "num":"' . $delivery_doc['invoice_no'] . '"}';

            $sign = $param . $key . $customer;
            $sign = md5($sign);
            $sign = strtoupper($sign);

            $_data = array('customer'=>$customer, 'param'=>$param, 'sign'=>$sign);
            $res = \Requests::post($url, array(), $_data);
            $data = json_decode($res->body);
            $delivery_data = $data->data;
            #echo '<pre>';var_dump($delivery_data);exit;
        }
        ...
        $this->assign('delivery_doc', $delivery_doc);
        $this->assign('delivery_status_text', $delivery_status_text);
    ...
    public function editshinn(){
      ...
      if(IS_POST)
     {
       ...
       $shipping = M('plugin')->where(array('status'=>1,'type'=>'shipping','code'=>I('shipping')))->find();

       M('order')->where(array('order_id'=>$order['order_id']))->save(array('shipping_name' =>$shipping['name'],'shipping_code' => $shipping['code']));

       M('delivery_doc')->where(array('order_id'=>$order['order_id']))->save(array('invoice_no'=>I('invoice_no')));
      ...
      $delivery_doc = M('delivery_doc')->where(array('order_id'=>$order_id))->order('id desc')->find();


      $this->assign('delivery_doc', $delivery_doc);
      $this->assign('order', $order);
      $this->assign('shipping_list', $shipping_list);
      // var_dump($order['shipping_code']);

        ...
        if($row){
            ...
            if($condition['type'] == 'shipping'){
                ...
                if($update['status']){
                    ...
                    # 添加快递
                    $_shipping = M('shipping')->where(array('shipping_code'=>$condition['code']))->find();
                    if ( empty($_shipping) ) {
                        $_plugin = M('plugin')->where(array('code'=>$condition['code']))->find();

                        $_add = array('shipping_code'=>$_plugin['code'],
                                      'shipping_name'=>$_plugin['name'],
                                      'shipping_desc'=>$_plugin['name'],
                                      'enabled'=>1);
                        M('shipping')->add($_add);
                    } else {
                        M('shipping')->where(array('shipping_code'=>$condition['code']))->update(array('enabled'=>1));
                    }
                }else{
                    M('shipping_area')->where(array('shipping_code'=>$condition['code']))->delete();

                    # 删除快递
                    M('shipping')->where(array('shipping_code'=>$condition['code']))->delete();
                }
            }
            $info['status'] = 1;

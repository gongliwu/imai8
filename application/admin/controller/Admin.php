...
class Admin extends Base {
        ...
        if(IS_POST){
            ...
            if(!empty($condition['user_name']) && !empty($condition['password'])){
                ...
                if(is_array($admin_info)){
                    ...
                    if($isSupplier){
                      cookie('workspaceParam','index|SupplierOrder',array('expire'=>1,'path'=>"/"));
                    }else{
                      $value = cookie('workspaceParam');
                      if($value == 'index|SupplierOrder'){
    ...
    public function role_info(){
		...
		$group = array('system'=>'系统设置','content'=>'内容管理','goods'=>'商品中心','member'=>'会员中心',
                ...
                'supplier_order'=>'供应商团购券', 'luck_draw'=>'抽奖管理'
		);

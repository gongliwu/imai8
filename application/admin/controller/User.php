...
class User extends Base {
     ...
     * 导入用户积分页面
     */
    public function import_points_page() {
        $data  = array();
        $error = array();
        ...
        if (IS_POST) {
            ...
            if ( ! empty($_FILES['points_file']['name']) ) {
                # 检查文件格式
                $ext_name = pathinfo($_FILES['points_file']['name'], PATHINFO_EXTENSION);
                ...
                $file_path_relatively = '/public/upload/tempxls/' . $file_name;
                $file_path_absolute = ROOT_PATH . 'public/upload/tempxls/' . $file_name;
                $ret = move_uploaded_file($_FILES['points_file']['tmp_name'], $file_path_absolute);
                if ( ! $ret ) {
                    $this->error('上传文件错误');
                }

                # 导入PHPExcel
                require_once ROOT_PATH . 'vendor/phpexcel/phpexcel/Classes/PHPExcel/IOFactory.php';

                # 解析表格
                $objPHPExcel = \PHPExcel_IOFactory::load($file_path_absolute);
                $arrExcel = $objPHPExcel->getSheet(0)->toArray();
                #array_shift($arrExcel);

                # 按行解析
                $i = 0;
                foreach ($arrExcel as $i => $v) {
                    $row = $i + 1;
                    ...
                    # 行数据不为空
                    if ($v['1'] != NULL || $v['4'] != NULL) {
                        $data[$i]['mobile'] = trim($v['1']);
                        $data[$i]['points'] = trim($v['4']);
                        ...
                        $data[$i]['user_id'] = 0;
                        $data[$i]['file_path'] = $file_path_relatively;
                        $data[$i]['add_time'] = $time;
                        $data[$i]['update_time'] = $time;

                        /**if ( empty($data[$i]['mobile']) && !empty($data[$i]['points'])) {
                            $this->error('积分' . $data[$i]['points'] . '的手机号码不能为空');
                        }

                        if ( empty($data[$i]['mobile']) && !empty($data[$i]['points'])) {
                            $this->error('手机' . $data[$i]['mobile'] . '的积分不能为空');
                        }**/

                        if ( empty($data[$i]['mobile']) ) {
                            $_data = array('name'=>$data[$i]['name'],
                        ...
                        if ( ! preg_match("/^[1-9]\d*$/", $data[$i]['points']) ) {
                            $_data = array('name'=>$data[$i]['name'],
                                           'mobile'=>$data[$i]['mobile'],
                                           'plate_number'=>$data[$i]['plate_number'],
                                           'premium'=>$data[$i]['premium'],
                ...
                if ( count($error) == 0 ) {
                    foreach ($data as $i => $row) {
                        $user = M('users')->where('mobile', $row['mobile'])->find();

                        if ( $user ) {
                            accountLog($user['user_id'], 0, $row['points'], '导入积分');
                            $data[$i]['user_id'] = $user['user_id'];
                        }

                        M('user_points_from_import')->add($row);
                    }
                }
            } else {
                $this->error("上传文件不存在");
     ...
     * 导入用户积分列表页面
     */
    public function import_points_list() {
        return $this->fetch();
    }
     ...
     * 导入用户积分列表页面
     */
    public function ajax_import_points_list() {
        // 搜索条件
        $condition = array();
        I('mobile') ? $condition['mobile'] = I('mobile') : false;
        #$sort_order = I('order_by', 'upfi_id').' '.I('sort', 'desc');

        $model = M('user_points_from_import');
        $count = $model->where($condition)->count();
        $Page  = new AjaxPage($count,10);
        //  搜索条件下 分页赋值
        foreach($condition as $key=>$val) {
        ...
        $upfiList = $model->where($condition)->order('upfi_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();

        $show = $Page->show();
        $this->assign('upfiList',$upfiList);
        $this->assign('page',$show);// 赋值分页输出
        $this->assign('pager',$Page);
        return $this->fetch();
    }

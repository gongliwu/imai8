...
class Index extends MobileBase {
    ...
    public function index(){
        ...
        */
        ...
        $work_partners = M('work_partner')->select();
...
//dump($seckill_list);die;
        ...
        $this->assign('work_partners',$work_partners);
    ...
    public function ajaxGetMore(){
    	$p = I('p/d',1);
    	$favourite_goods = M('goods')->where("is_recommend=1 and is_on_sale=1")->order('goods_id DESC')->page($p,18)->cache(true,TPSHOP_CACHE_TIME)->select();//首页推荐商品
    	$this->assign('favourite_goods',$favourite_goods);
    	return $this->fetch();
    }
     ...
     * 启用触发微信自动登录
     */
    public function enabled_third_login()
    {
        $dtl = M('disabled_third_login')->where('session_id', $this->session_id)->find();
        if ( empty($dtl) ) {
            M('disabled_third_login')->add(array('session_id'=>$this->session_id, 'is_disabled'=>0));
        } else {
            M('disabled_third_login')->where('session_id', $this->session_id)->update(array('is_disabled'=>0));
        }

...
class Goods extends MobileBase {
    ...
    public function goodsList(){
        ...
        Log::record("---------param start---------------", 'INFO', false);
        Log::record("brand_id:{$brand_id}", 'INFO', false);
        Log::record("spec:{$spec}", 'INFO', false);
        Log::record("attr:{$attr}", 'INFO', false);
        Log::record("price:{$price}", 'INFO', false);
        ...
        Log::record("filter_param:{$filter_param}", 'INFO', false);
        foreach ($filter_param as $k => $v) {
            Log::record("{$k} => {$v}", 'INFO', false);
        }
        Log::record("---------param end--------------", 'INFO', false);
    	...
    	$filter_price = $goodsLogic->get_filter_price($filter_goods_id,$filter_param,'goodsList', 5, $id); // 帅选的价格期间
    	$filter_brand = $goodsLogic->get_filter_brand($filter_goods_id,$filter_param,'goodsList',1, $id); // 获取指定分类下的帅选品牌
    	$filter_spec  = $goodsLogic->get_filter_spec($filter_goods_id,$filter_param,'goodsList',1); // 获取指定分类下的帅选规格
    	$filter_attr  = $goodsLogic->get_filter_attr($filter_goods_id,$filter_param,'goodsList',1, $id); // 获取指定分类下的帅选属性
    ...
    public function goodsInfo(){
        ...
        $spec_goods_price  = M('spec_goods_price')->where("goods_id", $goods_id)->getField("key,price,exchange_integral,store_count"); // 规格 对应 价格 库存表
...
//        $collect = M('goods_collect')->where("goods_id=$goods_id and user_id=$user_id")->count();
	...
	// 咨询列表
	 public function consultingList() {
	  ...
	  // 往期咨询列表 - ajax
	  public function ajax_consulting_list() {
	    $p = I('p/d', 1);
...
//      $show = $page->show();
        $list = M('GoodsConsult')->where($where)
        							...
        							->select();
        $replyList = M('GoodsConsult')->where("parent_id > 0")->where('goods_id', $goods_id)->order("id desc")->select();
...
//      echo '<pre>';var_dump($replyList,$list);exit;
    ...
    public function goodsConsult(){
        ...
        return array('result'=> 1,'msg'=>"咨询已提交");
    }
    /**
     * 商品详情页
     */
    ...
    public function ajaxComment(){
        ...
        $where = $goods_id ? "goods_id = {$goods_id} " : "goods_id > 0 ";
        if($commentType==5){
        	$where = $where . "and parent_id = 0 and img !='' ";
        }else{
        	$typeArr = array('1'=>'0,1,2,3,4,5','2'=>'4,5','3'=>'3','4'=>'0,1,2');
        	$where = $where . "and parent_id = 0 and ceil((deliver_rank + goods_rank + service_rank) / 3) in($typeArr[$commentType]) ";
        }
        $where = $where . "and ttype = {$ttype} ";

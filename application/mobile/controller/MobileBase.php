...
class MobileBase extends Controller {
    ...
    public function _initialize() {
        ...
        $is_login = 0;
        // 登录微信后，第一次进入手机首页 0表示从未进入首页，1表示第一次进入，2表示不是第一次进入
        if(session("is_first_mobile_index") == null){
          session('is_first_mobile_index',0);
        }

        $n_is_login = session('is_login');
        if(!isset($n_is_login) || empty($n_is_login)){
          session('is_login', 0);
        }
        $referurl = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : U("Mobile/User/index");
            ...
            if(is_array($this->weixin_config) && $this->weixin_config['wait_access'] == 1){
                ...
                if($data['status'] == 1){
                    ...
                    if(cookie("no_login_url") == null){
                      setcookie('no_login_url',$referurl,null,'/');
                    }
        ...
        $_user = session('user');
        $is_mobile_index = false;
        $referurl = strtolower($referurl);
        // if(!strstr($referurl,".html")){
        //   $referurl = substr($referurl,0,strlen($referurl)-1).".html";
        // }

        $indexs = [U("/mobile"),U("/mobile/index"),U("/mobile/index/index")];
        $is_has = !strstr($referurl,".html");
        foreach ($indexs as $key => $value) {
          if($is_has){
            $value = substr($value,0,strlen($value)-5);
          }
          if(strpos($referurl,$value) !== false){
            $is_mobile_index = true;
            break;
          }
        }

        // 判断是否登录，手机首页，第一次手机首页
        if(session('is_login') && $is_mobile_index && session("is_first_mobile_index") === 0){
          session('is_first_mobile_index',1);
        }

        if(session('is_login') && !$is_mobile_index && empty($_user['mobile']) && session("is_first_mobile_index") === 1)
        {
            $url = "index.php?m=mobile&c=index";
            if(strpos($referurl,$url) === false){
              session('is_first_mobile_index',2);
              $before_binging_url = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : U("Mobile/User/index");
              session('before_binging_url',$before_binging_url);
              Header("Location: " . U('Mobile/User/mobile_binding'));
              exit();
            }

...
class Cart extends MobileBase {
    ...
    function addCart()
    {
        ...
        $is_buy = I("is_buy/d", 0); // 是否直接购买 0:否 1:是
        $result = $this->cartLogic->addCart($goods_id, $goods_num, $goods_spec,$unique_id,$user_id, $is_buy); // 将商品加入购物车
    ...
    function ajaxAddCart()
    {
        ...
        $result = $this->cartLogic->addCart($goods_id, $goods_num, $goods_spec,$this->session_id,$this->user_id, $is_buy); // 将商品加入购物车
        exit(json_encode($result));
    }
    ...
    public function cart2()
    {
        ...
        if($cart_id){
          $result = $this->promptlyBuy($cart_id,$this->user,$this->session_id);
        }else{
    ...
    // 立即购买
    private function promptlyBuy($cart_id,$user,$session_id){
          $where = " 1 = 1 ";
          $cartWhere = "id = $cart_id ";

          $bind = array();
          if($user[user_id])// 如果用户已经登录则按照用户id查询
          {
               $where .= " and user_id = $user[user_id] ";
               $cartWhere .= " and user_id = $user[user_id] ";
               // 给用户计算会员价 登录前后不一样
          }
          else
          {
              $where .= " and session_id = :session_id";
              $cartWhere .= " and session_id = :session_id";
              $bind['session_id'] = $session_id;
              $user[user_id] = 0;
          }

          $cart = M('Cart')->where($cartWhere)->find();
          if(!$cart){
            return array('status'=>1,'msg'=>'','result'=>array('cartList' =>array(), 'total_price' => 0));
          }
          ...
          // $cartList[] = $cart;
          $anum = $total_price =  $cut_fee = $total_points = 0;

          foreach ($cartList as $k=>$val){
            $cartList[$k]['goods_fee'] = $val['goods_num'] * $val['member_goods_price'];
            $cartList[$k]['integral_fee'] = $val['goods_num'] * $val['exchange_integral'];
            ...
            $total_points += $val['goods_num'] * $val['exchange_integral'];
          }

          $total_price = array('total_fee' =>$total_price , 'cut_fee' => $cut_fee,'num'=> $anum,'total_points'=>$total_points); // 总计
    ...
    public function cart3(){
        ...
        $pay_type = I("pay_type/d"); // 支付方式 0:现金支付 1:能量支付
        ...
        $result  = calculate_price($this->user_id,$order_goods,$shipping_code,0,$address[province],$address[city],$address[district],$pay_points,$user_money,$coupon_id,$couponCode,$pay_type);
        ...
        if($_REQUEST['act'] == 'submit_order')
        {
            ...
            // 创建订单
            $result = $this->cartLogic->addOrder(
                $this->user_id,
                $address_id,
                $shipping_code,
                $invoice_title,

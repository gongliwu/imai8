...
function orderBtn($order_id = 0, $order = array())
{
    ...
    if ($order['luck_id'] > 0) {
        $luck_draw = M('luck_draw')->where('luck_id', $order['luck_id'])->find();

        # 是否开奖
        if ($luck_draw['status'] == 1) {
            $where = array('luck_id'=>$order['luck_id'], 'order_id'=>$order['order_id'], 'status'=>2);
            $luck_draw_user = M('luck_draw_user')->where($where)->find();

            # 是否中奖
            if ( ! empty($luck_draw_user) ) {
                // 货到付款
                if($order['pay_code'] == 'cod')
                {
...
function set_btn_order_status($order)
{
    ...
    if ($order['luck_id'] > 0) {
        ...
        # 是否开奖
        if ($luck_draw['status'] == 0) {
            $order['order_status_desc'] = '未开奖';
        } else {
            $where = array('luck_id'=>$order['luck_id'], 'order_id'=>$order['order_id'], 'status'=>2);
            $luck_draw_user = M('luck_draw_user')->where($where)->find();
...
function update_pay_status($order_sn,$ext=array())
{
	if(stripos($order_sn,'recharge') !== false){
        ...
        if (array_key_exists('integral', $ext)) {
            M('order')->where("order_sn", $order_sn)->save(array('pay_code'=>'integral','pay_name'=>'能量支付'));
        }
   ...
   *
   */
  function addCertificateCode($order_sn){
    $order = M('order')->where("order_sn",$order_sn)->find();
    ...
    for($i = 0;$i < $num;$i++){
      ...
      $code = isExistCertificateCode($code);
      ...
      M('certificate_code')->insertGetId($data);
   ...
   *
   */
  function isExistCertificateCode($code){
    $codeList = M('certificate_code')->getField("code_id,code_sn");
    ...
    if($isExist){
      return isExistCertificateCode(createCertificateCode());
    }
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '',$pay_type =0)
{
    ...
    foreach ($order_goods as $key => $val) {
        ...
        // if($pay_type == 1){
        //   $integral_money += $val['goods_num'] * $val['exchange_integral'];
        // }
        $integral_money += $val['goods_num'] * $val['exchange_integral'];
...
function substr_cut($user_name){
  ...
  if($strlen<2){
      return $user_name;
        ...
        if(preg_match("/1[3458]{1}\d{9}$/", $user_name)){
            //mb_substr — 获取字符串的部分
            $firstStr = mb_substr($user_name, 0, 3, 'utf-8');
            $lastStr = mb_substr($user_name, 7, 13, 'utf-8');
...
function get_social_time($timestamp)
{
    if ($timestamp <= 0) {
        return '';
    }
    ...
    if ($diff_time <= 0) {
        ...
        $ret = '刚刚';
    } elseif( $diff_time <= 1*60*60 ) {
        $ret = intval($diff_time/60) . '分钟前';
    } elseif( $diff_time <= 1*24*60*60 ) {
        $ret = intval($diff_time/(60*60)) . '小时前';
    } elseif( $diff_time <= 31*24*60*60 ) {
        $ret = intval($diff_time/(24*60*60)) . '天前';
    } elseif( $diff_time <= 365*24*60*60 ) {
        $ret = intval($diff_time/(30*24*60*60)) . '月前';
    } else {
        $ret = intval($diff_time/(365*24*60*60)) . '年前';
    }

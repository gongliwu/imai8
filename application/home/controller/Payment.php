...
class Payment extends Base {
     ...
     * 保险订单 提交支付方式
     */
    public function insuranceGetcode(){        
            //C('TOKEN_ON',false); // 关闭 TOKEN_ON
            header("Content-type:text/html;charset=utf-8");            
            $order_id = I('order_id/d'); // 订单id
            session('insurance_order',$order_id); // 最近支付的一笔订单 id
            // 修改订单的支付方式
            $payment_arr = M('Plugin')->where("`type` = 'payment'")->getField("code,name");
            M('insurance_order')->where("order_id",$order_id)->save(array('pay_method'=>$payment_arr[$this->pay_code]));
           ...
           $this->assign('code_str', $code_str); 
           $this->assign('order_id', $order_id);           
           return $this->fetch('insurance_payment');  // 分跳转 和不 跳转 
    }

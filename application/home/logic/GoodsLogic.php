...
class GoodsLogic extends Model
{
     ...
     * 获取 商品列表页帅选品牌
     */
    public function get_filter_brand($goods_id_arr, $filter_param, $action, $mode = 0, $cat_id=0)
    {
        ...
        if ($cat_id == 0) {
            ...
            $list_brand = M('brand')->where(array('cat_id'=>$cat_id))->limit('30')->select();
        }
     ...
     * 获取商品列表页帅选属性
     */
    public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0, $cat_id=0)
    {
        if ($cat_id == 0) {
            $goods_id_str = implode(',', $goods_id_arr);
            $goods_id_str = $goods_id_str ? $goods_id_str : '0';
            ...
            $goods_attr = M('goods_attr')->where(array('attr_value' => ['<>', '']))
                               ->join('tp_goods', 'tp_goods.goods_id = tp_goods_attr.goods_id and tp_goods.cat_id='.$cat_id)
                               ->select();
        }
        ...
        foreach ($goods_attr as $k => $v) {
            ...
            $list_attr[$v['attr_id']]['attr_value'][] = array(
                    'key' => $v['attr_id'],
                    'val' => $v['attr_value'],
     ...
     * @return array
     */
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5, $cat_id=0)
    {
        if (!empty($filter_param['price']) && $cat_id==0)
            return array();
        ...
        if ($cat_id == 0) {
            ...
            $priceList = M('goods')->where(array('cat_id' => $cat_id))
                                   ->getField('shop_price', true);
        }
    ...
    function getGoodsIdByAttr($attr)
    {
        ...
        Log::record("---------getGoodsIdByAttr start--------------", 'INFO', true);
        ...
        $whereArr = array();
        $whereArr['attr_id'] = array('in', $attr_id);
        $whereArr['attr_value'] = array('in', $attr_value);
        $c = count($attr_id) - 1;
        if ($c > 0)
            ...
            $arr = M('goods_attr')->where($whereArr)->group('goods_id')->having("count(goods_id) > $c")->getField("goods_id", true);  //select * from   `tp_goods_attr` where attr_id in(59,80) and attr_value in('直板','翻盖','蓝牙4.0') group by goods_id having count(goods_id) > 1
        else
            // $arr = M('goods_attr')->where($where)->bind($bind)->getField("goods_id", true); // 如果只有一个条件不再进行分组查询
            $arr = M('goods_attr')->where($whereArr)->getField("goods_id", true); // 如果只有一个条件不再进行分组查询

...
class CartLogic extends Model
{
     ...
     * @param type $is_buy 是否直接购买 0:否 1:是
     */
    function addCart($goods_id,$goods_num,$goods_spec,$session_id,$user_id = 0, $is_buy=0)
    {
        ...
        if(!empty($specGoodsPriceList) && empty($goods_spec)) // 有商品规格 但是前台没有传递过来
            return array('status'=>-1,'msg'=>'请选择商品规格','result'=>'');
        if($goods_num <= 0)
       ...
       if($catr_goods)
       {
           // 如果购物车的已有数量加上 这次要购买的数量  大于  库存输  则不再增加数量
            ...
            if ($is_buy == 1) {
                M('Cart')->where(array(
                'user_id' => $user_id,
                'id' => array('neq', $catr_goods['id'])))->setField('selected', 0);

                M('Cart')->where(array('user_id' => $user_id, 'id' => $catr_goods['id']))->setField('selected', 1);
            }
       ...
       else
       {
            ...
            if ($is_buy == 1) {
                M('Cart')->where(array(
                    'user_id' => $user_id,
                    'id' => array('neq', $insert_id)))->setField('selected', 0);

                M('Cart')->where(array('user_id' => $user_id, 'id' => $insert_id))->setField('selected', 1);
            }
     ...
     */
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$user_note='', $pay_type=0)
    {
...
//                'integral_money'   =>$car_price['pointsFee'],//'使用积分抵多少钱',
                'integral_money'   => $car_price['pointsFee'],
        ...
        return array(
                ...
                'msg'=>'提交订单成功',
                'result'=>$result,
                'order_info'=>$ret_order_info); // 返回新增的订单id
    }

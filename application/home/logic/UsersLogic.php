...
class UsersLogic extends Model
{
    ...
    public function get_order_goods($order_id){
        $sql = "SELECT og.*, g.exchange_integral FROM __PREFIX__order_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id WHERE order_id = :order_id";
...
//        $type = I('get.type',0);
    ...
    public function get_goods_collect($user_id){
        ...
        $show = $page->show();
        //获取我的收藏列表
        $sql = "SELECT c.collect_id,c.add_time,g.goods_id,g.goods_name,g.shop_price,g.original_img,g.is_on_sale,g.exchange_integral FROM __PREFIX__goods_collect c ".
     ...
     * @return mixed
     */
    public function get_comment($user_id,$status=2,$rec_id=0,$is_send=1){
        if($status == 1){
            //已评论
            ...
            $sql = "select c.*,g.*,(select order_sn from  __PREFIX__order where order_id = c.order_id ) as order_sn  from  __PREFIX__comment as c inner join __PREFIX__order_goods as g on c.goods_id = g.goods_id and c.order_id = g.order_id where c.user_id = $user_id order by c.add_time desc LIMIT {$page->firstRow},{$page->listRows}";
	        	...
	        	left join __PREFIX__order as o on o.order_id = og.order_id where o.user_id = $user_id  ";
	        	$where = '';
	        	if($status == 0){
	        		$countsql .= $where = " and og.is_comment = 0 ";
	        	}
	        	if($rec_id != 0) {
	        		$countsql .= " and og.rec_id = {$rec_id} ";
	        		$where .= " and og.rec_id = {$rec_id} ";
	        	}
                if($is_send == 1) {
                    $countsql .= " and og.is_send = 1 ";
	            ...
	            left join __PREFIX__order as o on o.order_id = og.order_id  where o.user_id = $user_id $where order by o.order_id desc  LIMIT {$page->firstRow},{$page->listRows}";
        }
    ...
    public function add_comment($add){
        ...
        if($order['order_status'] != 2 && $order['order_type'] != 2)
        ...
        // 检查类型
        if ( ! in_array($add['ttype'], array(1,2)) ) {
            return array('status'=>-1, 'msg'=>'错误的类型','result'=>'');
        }

        // 抽奖
        if ( $add['ttype'] == 2 ) {
            $luck_draw = M('luck_draw')->where('luck_id', $add['luck_id'])->find();
            if ( empty($luck_draw) ) {
                return array('status'=>-1,'msg'=>'抽奖不存在', 'result'=>'');
            }

            $luck_tpl = M('luck_tpl')->where('tpl_id', $luck_draw['tpl_id'])->find();
            if ( empty($luck_tpl) ) {
                return array('status'=>-1,'msg'=>'抽奖模板不存在', 'result'=>'');
            }

            $add['goods_name'] = $luck_tpl['goods_name'];
            $add['period'] = $luck_draw['period'];
        }
    ...
    public function password($user_id,$old_password,$new_password,$confirm_password,$is_update=true) {
        ...
        // 检查 - 更新密码 - 验证原密码
        if ($is_update==true && encrypt($old_password)!=$user['password'])
